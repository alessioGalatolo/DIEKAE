...
def main():
    ...
    model = ClassificationLlama4Encoders.from_pretrained(
        model_name, device_map="cpu", use_cache=False, torch_dtype=torch.bfloat16, label2id=ICRDataset.token_to_number, id2label=ICRDataset.number_to_token, num_labels=len(ICRDataset.token_to_number)
    )
    ...
    for dataset_name in DATASETS_NAMES:
        data_test = ICRDataset(
            path.join(args.data_path, dataset_name), tokenizer, "val", limit_number_of_samples=1000
        )
